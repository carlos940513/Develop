...
public class AddColumnGenerator extends AbstractSqlGenerator<AddColumnStatement> {
    ...
    protected String generateSingleColumnSQL(AddColumnStatement statement, Database database) {
        ...
        if (!statement.isNullable()) {
            ...
            if (database instanceof SybaseDatabase || database instanceof SybaseASADatabase || database instanceof MySQLDatabase|| (database instanceof MSSQLDatabase && columnType.toString().equals("timestamp"))) {
                alterTable += " NULL";
            }
        }
